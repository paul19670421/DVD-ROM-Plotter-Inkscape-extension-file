...
class LaserGcode(inkex.Effect):
    ...
    def export_gcode(self, gcode):
        ...
        f.write(
            self.options.laser_off_command + " S50" + "\n" + self.header +
            "\n" + gcode + self.footer)
        f.close()
    ...
    def __init__(self):
        ...
        self.arguments = [
            ...
            {"name": "--SG90-command", "type": str, "dest": "laser_command",
             "default": "M300", "help": "Laser gcode command"},

            {"name": "--SG90-off-command", "type": str, "dest": "laser_off_command",
             "default": "M300", "help": "Laser gcode end command"},

            {"name": "--pen-speed", "type": int, "dest": "laser_speed", "default": 750,
             "help": "Laser speed (mm/min},"},

            {"name": "--travel-speed", "type": str, "dest": "travel_speed",
             "default": "3000", "help": "Travel speed (mm/min},"},

            {"name": "--SG90 down angle", "type": int, "dest": "laser_power", "default": 255,
            ...
            if s[1] == 'move':
                ...
                if (r[0] ** 2 + r[1] ** 2) > .1:
                    ...
                    if abs(r1.mag() - r2.mag()) < 0.001:
                        g += ("G1" if s[3] < 0 else "G1") + c(
                            si[0] + [None, (s[2][0] - s[0][0]), (s[2][1] - s[0][1])]) + "\n"
                    else:
                        r = (r1.mag() + r2.mag()) / 2
                        g += ("G1" if s[3] < 0 else "G1") + c(si[0]) + " R%f" % (r) + "\n"
                    lg = 'G02'
                else:
                    g += "\n"
                    lg = 'G01'
